...
- the tracked hardened paper-evidence snapshot is:
...
| `E-P2-INTEGRITY` | `R4` Phase 2 artifact integrity | Phase 2 deepening cycle `phase2_deepening_cycle_20260327T155853Z`; tracked snapshot `docs/research/evidence/phase2_20260327_summary.md` | qualified homogeneous `RTX A6000` pool | the completed Phase 2 chain is internally consistent: all four campaigns finished successfully, all five studies emitted their full report sets, and the tracked summary preserves the promoted integrity and claim readout even if the large artifact tree is removed locally | High | this row only proves completeness and provenance, not scientific direction by itself | No |
...
| `E-P3-INTEGRITY` | `R5` Phase 3 artifact integrity | canonical confirmation studies plus tracked snapshot `docs/research/evidence/phase3_20260329_summary.md` and `docs/research/evidence/phase3_20260329/` | qualified homogeneous `RTX A6000` pool | the completed Phase 3 program is internally consistent: all canonical and replication campaigns finished successfully, all promoted studies emitted their expected report sets under the diagnostic/reportable rules, and the tracked snapshot records the transfer failure plus the `split_k` and `rows_per_program` retirement decisions | High | this row proves completeness and provenance, not scientific direction by itself | No |
...
| `E-R6-INTEGRITY` | `R6` final artifact integrity | `gemm_final_baseline_mapping` campaign `run_20260330T003313Z_9e0cdfce`; `gemm_final_selector_ablation` campaign `run_20260330T014321Z_c4e9fa9d`; tracked snapshot `docs/research/evidence/final_paper_20260403/` | qualified homogeneous `RTX A6000` pool | the bounded final-mainline program completed cleanly: both required campaigns finished with `0` failures, the optional aligned refresh and confirmation reruns were skipped by explicit gate, and the tracked paper snapshot now pins the promoted sources into repo-local files that survive local artifact cleanup | High | this row proves completeness and promotion discipline rather than scientific direction by itself | No |
...
| `E-R6-BUNDLE` | final paper-evidence lock | `docs/research/evidence/final_paper_20260403/` and [11_final_claim_inventory.md](11_final_claim_inventory.md) | repository-local promoted evidence snapshot | the final paper-evidence package is reproducible from tracked repo state: the figure source map, final claim table, headline summary, plot-ready figure CSVs, and final claim inventory all agree on one narrow figure set and one final wording discipline | High | if a future paper draft needs a figure not covered by this snapshot, that request should be treated as a provenance gap rather than as justification for a new exploratory run | No |
