...
- the current final state is now evaluated rather than pending: the transfer-safe corrective pass has completed, the bounded keep/drop decisions are known, and the guarded non-`split_k` mainline lock has closed the last paper-facing selector path
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `OPP-001` | representative GEMM frontier construction was the dominant selector weakness, but the safe corrective path is now closed | compile-frontier ranking was sensitive both to underweighting balanced high-area tiles and to overcorrecting toward oversized masked tiles once the space expanded | earlier evidence showed the selector missing the balanced `128x128x32` family; the Phase 2 expanded space showed collapse toward oversized masked tiles; the final R6 guarded frontier union recovered the mainline result on the non-`split_k` surface | representative GEMM, especially `m_dominant`, `square_compute`, and `edge_nondivisible` classes | replace the current area-seeking frontier rule with a conservative, shape-aware guarded frontier rather than another broad heuristic family | Yes, three times | narrow-space retry worked, expanded-space v2/v3 revisions failed, and the final guarded non-`split_k` mainline pass recovered a bounded positive result | Close as resolved for the current paper-facing program; do not reopen it as a new selector-family expansion |
...
| `OPP-008` | the Phase 2 GEMM revision overgeneralized toward oversized masked tiles | the frontier rule rewarded tile area and balance without enough penalty for excessive masking or shape-relative overcoverage | `gemm_v2_baseline_mapping` and `gemm_v2_selector_ablation` both showed `prune_rank_revised` selecting `cfg_24d8f84f938f` (`256x256x32`, `group_size_m=8`, `num_stages=4`, `num_warps=8`) across all representative workload classes, with catastrophic held-out performance | representative GEMM expanded v2 space | add a shape-relative tile-fit term or an explicit masked-oversize penalty before admitting any further GEMM revised-selector retry | Yes | the completed Phase 3 corrective pass validated the diagnosis, and the final R6 guarded non-`split_k` union frontier resolved it for the paper-facing mainline surface | Close as resolved failure-analysis evidence |
...
- any new selector-family expansion after the completed final mainline lock
