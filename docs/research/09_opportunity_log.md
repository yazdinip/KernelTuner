...
- that is the current Phase 3 state: the transfer-safe corrective pass is implemented, but Phase 3 execution is not yet promoted into the research-facing docs
...
- persistent scheduling or broader algorithm-family expansion before the bounded Phase 3 `split_k` pass is evaluated
