...
class RunStore:
    ...
    @classmethod
    def from_run_dir(cls, run_dir: str | Path) -> "RunStore":
        resolved = Path(run_dir).resolve()
        if not resolved.parent.parent.exists():
            raise FileNotFoundError(f"invalid run directory '{resolved}'")
        store = cls(
        ...
        if store.manifest_path.exists():
            store._manifest = store.load_manifest()
        return store
    ...
    def write_summary(self, result: ExperimentResult) -> Path:
        return self.write_json_artifact("summary", result.model_dump(mode="json"), filename="summary.json")
    ...
    def write_json_artifact(
        ...
        *,
        ...
        path = self.run_dir / (filename or f"{logical_name}.json")
    ...
    def write_csv_artifact(
        ...
        *,
        ...
        path = self.run_dir / (filename or f"{logical_name}.csv")
        self._write_csv_atomic(path, frame)
        self._register_artifact(
            ArtifactFile(
                logical_name=logical_name,
                relative_path=path.relative_to(self.run_dir).as_posix(),
    ...
    def _write_csv_atomic(self, path: Path, frame: pd.DataFrame) -> None:
        fd, tmp_name = tempfile.mkstemp(dir=path.parent, suffix=".tmp")
        os.close(fd)
        tmp_path = Path(tmp_name)
        try:
